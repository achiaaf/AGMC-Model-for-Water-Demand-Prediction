...
# def linear_model(x, y, z):
#     """where x is what we end up predicting"""
#     a = len(x)
#     b = len(z) - a
#     X = np.vstack([np.ones(a), y[:, a-1], z[:, a]])
#     p = inv(X.T @ X) @ X.T @ x
#     X1 = np.vstack([np.])
def mse(x, y):
...
def grey_degree2(X, Y):
    ...
    # Calculating the grey relational degree from the grey relational coefficient
    # 0.5 is the distinguishing coefficient taken from literature
    grey_degree_Y = np.mean((np.min(diff) + 0.5 * np.max(diff)) / (diff + 0.5 * np.max(diff)))
